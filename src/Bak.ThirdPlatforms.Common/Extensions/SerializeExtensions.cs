...
namespace Bak.ThirdPlatforms.Common.Extensions
{
    public static class SerializeExtensions
    {
        ...
        public static async Task<T> FromJsonFile<T>(this string filePath, string key = "") where T : new()
        {
            ...
            if (string.IsNullOrEmpty(key)) return JsonConvert.DeserializeObject<T>(jsonStr);

            return !(JsonConvert.DeserializeObject<object>(jsonStr) is JObject obj) ? new T() : JsonConvert.DeserializeObject<T>(obj[key].ToString());
        }
    }
}
